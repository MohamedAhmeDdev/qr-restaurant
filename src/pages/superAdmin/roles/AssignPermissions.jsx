...
export default function AssignPermissions() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 bg-slate-50 dark:bg-slate-950 min-h-screen text-slate-900 dark:text-slate-100 transition-colors duration-200">
      
      {/* HEADER SECTION */}
      <div className="border-b border-slate-200 dark:border-slate-800 pb-5 transition-colors duration-200">
        <Link
          to="/roles"
          className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors duration-200 mb-4"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          {!isPageLoading && (
            <span className={`text-xs px-2.5 py-0.5 rounded-full font-semibold transition-colors duration-200 ${
              roleInfo.isSystem 
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 transition-colors duration-200">
          Configuring capability privileges for <strong className="text-slate-900 dark:text-white transition-colors duration-200">{roleInfo.name || 'Role'}</strong>
        </p>
      </div>

      {/* ERROR ALERT */}
      {apiError && (
        <div className="p-4 rounded-xl bg-rose-50 dark:bg-rose-950 border border-rose-200 dark:border-rose-900 text-rose-700 dark:text-rose-400 flex items-center justify-between text-sm transition-colors duration-200">
          <div className="flex items-center gap-2">
            <AlertCircle className="w-5 h-5 shrink-0" />
            <span>{apiError}</span>
          </div>
          <button onClick={fetchData} className="text-xs font-semibold underline hover:no-underline transition-colors duration-200">
            Retry
          </button>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm transition-colors duration-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-sm text-slate-900 dark:text-white transition-colors duration-200">
          ...
          {groupsLoading ? (
            <div className="flex items-center gap-1 px-3 py-1.5">
              {[...Array(4)].map((_, idx) => (
                <div 
                  key={idx} 
                  className="h-7 w-16 bg-slate-200 dark:bg-slate-700 rounded-md animate-pulse"
                />
              ))}
            </div>
          ) : (
          ...
          {Object.entries(groupedPermissions).map(([category, perms]) => (
            <div key={category} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors duration-200">
              ...
              <div className="px-6 py-3.5 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between transition-colors duration-200">
                <div className="flex items-center gap-2 text-slate-800 dark:text-slate-200 font-bold text-sm transition-colors duration-200 capitalize">
                  <span>{category} Permissions</span>
                </div>
                <span className="text-xs font-medium text-slate-500 dark:text-slate-400 transition-colors duration-200">
                        ...
                        <div 
                          className={`mt-0.5 w-5 h-5 rounded flex items-center justify-center border transition-colors duration-200 ${
                            isChecked 
                              ...
                              : 'border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-orange-400'
                          }`}
                        >
                          {isChecked && <Check className="w-3.5 h-3.5 stroke-[3]" />}
                        </div>
                        <div>
                          <p className="font-semibold text-sm text-slate-900 dark:text-white transition-colors duration-200">{perm.name}</p>
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 transition-colors duration-200">{perm.description || perm.slug}</p>
                        </div>
                      </div>
